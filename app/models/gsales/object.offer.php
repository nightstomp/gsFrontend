...
class GSALES2_OBJECT_OFFER extends GSALES2_OBJECT {
	...
	public function __construct($arrAPIResult=''){
		if (is_object($arrAPIResult)){
			...
			foreach ((array)$arrAPIResult->pos as $key => $arrValuePos) $this->pos[$arrValuePos->id] = new GSALES2_OBJECT_OFFER_POS($arrValuePos); // pos
			$this->summ = new GSALES2_OBJECT_OFFER_SUMM($arrAPIResult->summ); // summ
			
			if ($arrAPIResult->dunning){
				foreach ((array) $arrAPIResult->dunning as $key => $arrValueDunning){
					$objDunning = new GSALES2_OBJECT_OFFER_DUNNING($arrValueDunning); // dunning information
	...
	public function getStatusIdAsText(){
		$arrStatus[0] = 'offen';
		$arrStatus[1] = 'storniert';
		$arrStatus[2] = 'angenommen';
		$arrStatus[3] = 'abgerechnet';
...
class GSALES2_OBJECT_OFFER_POS extends GSALES2_OBJECT {
...
class GSALES2_OBJECT_OFFER_SUMM extends GSALES2_OBJECT {
...
class GSALES2_OBJECT_OFFER_DUNNING extends GSALES2_OBJECT {
